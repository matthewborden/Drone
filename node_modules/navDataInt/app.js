...
function processNavdata(navData.demo) {
	logData('./tmp/data.txt', JSON.stringify(navData));
	logData('./tmp/test.txt',"[" + navData.demo.altitudeMeters + "," + navData.demo.batteryPercentage + "," + (new Date() - oldTime) + "]");
	if (navData.demo.altitudeMeters < 2) drone.up(0.1);
	if (navData.demo.altitudeMeters > 1) runEffects();
	...
	newSpeed = (0.4 * cte); // - (1 * change / time) - (1 * cte_sum)
...
function logData (filename, dataToBeLogged) {
	...
	fs.readFile(filename, 'utf8', function (err,data) {
	  ...
	  fs.writeFile(filename, data + dataToBeLogged, function(err) {
		    if(err) {
		        console.log(err);
		    } 
		}); 
